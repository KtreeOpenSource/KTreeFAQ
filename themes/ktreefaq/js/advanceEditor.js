...
(function ($) {
    $.fn.pageEditor = function (options, callback) {
        ...
        $this.on('click', '.media-upload', function (event) {
            button = $(this);

            if ($('#media-editor-modal').length == 0) {
                //$('body').append($this.mediaForm(event));
		//$(".modal-backdrop").remove();
		  $('.media-model').show();
            }
            ...
            $('#media-editor-modal #mediainput').bind('change', function (event) {
                ...
                file = input.val();
		if (file != undefined) {                
			button.trigger("mediaReceieved", file);
        ...
        $this.on('mediaReceieved', '.media-upload', function (e, data) {
            var imageUrl = data;

            var thisVariable = $(this);
                    $image = $('<img/>').attr({'src': imageUrl, 'align': 'center'}).css({'width': '100%'});
